...
def task():
    ...
    for i in range(len(tasks)):
        ...
        target = task_array[len(task_array) // 2] # Position will be randomized after shuffle
        total_time_linear = timeit.timeit(lambda: linear_search_task(random.sample(task_array, len(task_array)), 5), number=100)
        total_time_binary = timeit.timeit(lambda: binary_search_task(random.sample(task_array, len(task_array)), 5, 0, len(task_array) - 1), number=100)
...
# Linear search is faster at any input size since having to first sort the array in, at best, logarithmic time
# and then searching in logarithmic time takes longer than just naively searching through the array. 
# Both functions appear to have a linear time complexity, though the slope for linear search is much smaller.
